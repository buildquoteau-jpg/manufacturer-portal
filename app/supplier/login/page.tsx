...
export default function SupplierLoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (supplier?.slug) {
      // If user didn't tick "Remember me", mark session as tab-only.
      // The supplier portal will clear the persisted token on browser close.
      if (!rememberMe) {
        sessionStorage.setItem('mfp-session-only', '1')
      } else {
        sessionStorage.removeItem('mfp-session-only')
      }
        ...
        {view === 'login' && (
          <form onSubmit={handleLogin} className="bg-surface border border-border rounded-2xl p-8 space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  placeholder="••••••••"
                  required
                  className="w-full bg-ui border border-border rounded-lg px-3 py-2.5 pr-10 text-text-primary placeholder-text-faint text-sm focus:outline-none focus:border-brand transition-colors"
                />
                <button
                  type="button"
                  tabIndex={-1}
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <label htmlFor="rememberMe" className="text-xs text-text-faint cursor-pointer select-none">
